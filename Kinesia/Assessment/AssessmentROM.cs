...
namespace Kinesia.Assessment
{
    ...
    public partial class AssessmentROM : Form
    {
        ...
        private void AssessmentROM_Load(object sender, EventArgs e)
        {
            ...
            // --- Start the main processing timer --- (FIXED)
        ...
        private void cmbMovementSelection_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (_currentState != MeasurementState.Idle) // (FIXED)
            {
        ...
        private void SdkTimer_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (cf != null && cf.Width > 0 && cf.DataPtr != IntPtr.Zero)
                {
                    ...
                    if (_colorBuffer == null || _colorBuffer.Length != colorLength) // (FIXED)
                    {
                        _colorBuffer = new byte[colorLength];
                    }
                    cf.CopyData(ref _colorBuffer);

                    for (int i = 0; i < _colorBuffer.Length; i += 3) // (FIXED)
                    {
                ...
                if (df != null && df.Width > 0 && df.DataPtr != IntPtr.Zero && _smoothedJoints != null && cf != null)
                {
                    ...
                    if (_depthBuffer == null || _depthBuffer.Length != depthCount) // (FIXED)
                    {
                ...
                if (cf != null && _colorBuffer != null) // (FIXED)
                {
        ...
        private void btnStartStopMeasurement_Click(object sender, EventArgs e)
        {
            switch (_currentState)
            {
                case MeasurementState.Idle:
                    if (_currentMovement == MovementType.None) // (FIXED)
                    {
        ...
        // Smooth3DJointsMovingAverage (FIXED)
        private Point3D[] Smooth3DJointsMovingAverage(Point3D[] newJoints)
        {
            if (newJoints == null || newJoints.Any(p => p.Z <= 0))
            {
        ...
        // SmoothAngleMovingAverage (FIXED)
        ...
        private void RenderColorWithPose(int w, int h, byte[] buffer, PointF[] joints2D, Point3D[] joints3D_smoothed)
        {
            ...
            try
            {
                ...
                {
                    if (pictureBoxRgb.Image != null) pictureBoxRgb.Image.Dispose();
            ...
            {
                /* Don't dispose bmp wrapping Marshal pointer */
            }
        }
        ...
        private void DrawLimbPose(Graphics g, PointF[] joints2D, Point3D[] joints3D_smoothed)
        {
            ...
            if (joints3D_smoothed != null && joints3D_smoothed.Length > 1 && joints3D_smoothed[1].Z > 0)
            {
                ...
                else limbColor = Color.Red;
            }
            _lastConfidenceColor = limbColor;
            _bonePen.Color = limbColor;
            if (joints2D.Length > activeIndices[0] && !joints2D[activeIndices[0]].IsEmpty && joints2D.Length > activeIndices[1] && !joints2D[activeIndices[1]].IsEmpty)
            {
                g.DrawLine(_bonePen, joints2D[activeIndices[0]], joints2D[activeIndices[1]]);
            }
            if (joints2D.Length > activeIndices[1] && !joints2D[activeIndices[1]].IsEmpty && joints2D.Length > activeIndices[2] && !joints2D[activeIndices[2]].IsEmpty)
            {
            ...
            for (int i = 0; i < activeIndices.Length; i++)
            {
                ...
                if (joints2D.Length > jointIndex && !joints2D[jointIndex].IsEmpty)
                {
                    ...
                    Color jointColor = isOccluded ? Color.Red : Color.White;
        ...
        // AssessmentROM_FormClosing (FIXED)
